    ...
    const initializeUser = async () => {
        if(program && publicKey){
            try{
                ...
                const [profilePda, profileBump] = await findProgramAddressSync([utf8.encode('USER_STATE'), publicKey.toBuffer()], program.programId)
                const tx = await program.methods.initializeUser().accounts({userProfile: profilePda, authority: publicKey, SystemProgram: SystemProgram.programId}).rpc()
    ...
    const addTodo = async (e) => {
        ...
        if(program && publicKey){
            try{
                ...
                if(input) {
                    await program.methods.addTodo(input).accounts({userProfile: profilePda, todoAccount: todoPda, authority: publicKey, SystemProgram: SystemProgram.programId}).rpc()
                }
    ...
    const markTodo = async (todoPda, todoIdx) => {
        if (program && publicKey) {
            try {
                ...
                await program.methods.markTodo(todoIdx).accounts({userProfile: profilePda, todoAccount: todoPda, authority: publicKey, systemProgram: SystemProgram.programId}).rpc()
